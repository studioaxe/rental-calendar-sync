...
def create_master_calendar(events: List[Dict]) -> Calendar:
    """Cria calendario master"""
    logger.info("PASSO 5: Criacao do calendario master...")
    ...
    for event in events:
        try:
            ...
            # ═══════════════════════════════════════════
            # EVENTO 1: RESERVA IMPORTADA (Data Original)
            # ═══════════════════════════════════════════
            
            reserva_event = Event()
            reserva_event.add('uid', f"{uid_base}")
            reserva_event.add('summary', f"{summary}")
            reserva_event.add('dtstart', dtstart)
            reserva_event.add('dtend', dtend)
            reserva_event.add('description', f"[{source}]\nReserva Importada de {source}")
            ...
            master.add_component(reserva_event)
            event_count += 1
            
            logger.debug(f"   Evento 1 (Reserva): {summary} [{to_date(dtstart)} - {to_date(dtend)}]")
            
            # ═══════════════════════════════════════════
            # EVENTO 2: TP ANTES (Bloqueio) - VALUE=DATE!
            # ═══════════════════════════════════════════
            
            tp_before_start_date = to_date(dtstart - timedelta(days=BUFFER_DAYS_BEFORE))
            tp_before_end_date = to_date(dtstart)
            
            tp_before_event = Event()
            tp_before_event.add('uid', f"{uid_base}-tp-before")
            tp_before_event.add('summary', f"TP Antes: {summary}")
            tp_before_event.add('dtstart', tp_before_start_date)  # VALUE=DATE
            tp_before_event.add('dtend', tp_before_end_date)      # VALUE=DATE
            tp_before_event.add('description', f"[{source}-TP-BEFORE]\nTempo de Preparacao ANTES (bloqueio {BUFFER_DAYS_BEFORE} dia(s))")
            ...
            master.add_component(tp_before_event)
            event_count += 1
            
            logger.debug(f"   Evento 2 (TP Antes VALUE=DATE): [{tp_before_start_date} - {tp_before_end_date}]")
            
            # ═══════════════════════════════════════════
            # EVENTO 3: TP DEPOIS (Bloqueio) - VALUE=DATE!
            # ═══════════════════════════════════════════
            
            tp_after_start_date = to_date(dtend)
            tp_after_end_date = to_date(dtend + timedelta(days=BUFFER_DAYS_AFTER))
            
            tp_after_event = Event()
            tp_after_event.add('uid', f"{uid_base}-tp-after")
            tp_after_event.add('summary', f"TP Depois: {summary}")
            tp_after_event.add('dtstart', tp_after_start_date)   # VALUE=DATE
            tp_after_event.add('dtend', tp_after_end_date)       # VALUE=DATE
            tp_after_event.add('description', f"[{source}-TP-AFTER]\nTempo de Preparacao DEPOIS (bloqueio {BUFFER_DAYS_AFTER} dia(s))")
            ...
            master.add_component(tp_after_event)
            event_count += 1
            
            logger.debug(f"   Evento 3 (TP Depois VALUE=DATE): [{tp_after_start_date} - {tp_after_end_date}]")
            
        except Exception as e:
            logger.error(f"[ERRO] ao adicionar evento: {e}")
            continue
    
    logger.info(f"   [OK] Calendario master criado: {event_count} eventos ({len(events)} reservas)")
    return master

def export_to_file(master: Calendar, filename: str = OUTPUT_FILE) -> bool:
    """Exporta calendario para ficheiro .ics"""
    try:
        logger.info(f"PASSO 6: Exportacao para {filename}...")
        
        with open(filename, 'wb') as f:
            f.write(master.to_ical())
        
        file_size = os.path.getsize(filename)
        logger.info(f"   [OK] Ficheiro exportado: {filename} ({file_size} bytes)")
...
def main() -> int:
    ...
    try:
        ...
        master = create_master_calendar(events)
        if not export_to_file(master):
            msg = "Erro ao exportar ficheiro"
            log_end(False, msg)
            send_error_email(msg)
            return 4
