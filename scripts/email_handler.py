...
📁 FICHEIRO: master_calendar.ics
  └─ Disponível em GitHub Artifacts

🚀 PRÓXIMOS PASSOS:
  1. Descarregar master_calendar.ics
  2. Importar em Airbnb
  3. Importar em Booking
  4. Importar em Vrbo
...
"""
    ...
    def send_error(self, error_msg: str, log_file: str = 'sync.log') -> bool:
        ...
        Returns:
            True se sucesso
        """
        subject = f'❌ Erro na Sincronização Calendários - {datetime.now().strftime("%d/%m/%Y %H:%M")}'
...
"""
    ...
    def send_daily_report(self, report_data: dict) -> bool:
        ...
        """
        subject = f'📊 Relatório Sincronização - {datetime.now().strftime("%d/%m/%Y")}'
